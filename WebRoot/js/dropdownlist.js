...
(function($){
	$.fn.dropdownlist=function(opts){
	   ...
	   // var selectedvalue=  $('<input>',{type:'hidden',id:settings.selectedvalue,name:settings.selectedvalue}).appendTo($this);      
		//	if(settings.checkbox)
		//{
		...
		//};
				...
				$.each(mdata,function(k,v){					
		            ...
		            $cell.attr('value',k).css({'cursor':'pointer'}).click(function(evt){
						...
						if(settings.checkbox){
							...
							tmptext = typeof(tmptext)!='undefined'?(tmptext!=''?tmptext.split(';'):[]):(objid.text!=''?objid.text.split(';'):[]);
							tmpvalue = typeof(tmpvalue)!='undefined'?(tmpvalue!=''?tmpvalue.split(';'):[]):(objid.value!=''?objid.value.split(';'):[]);
					...
					if(typeof(select.attr('selectedTmpValue'))!='undefined'){
						select.attr({'selectedText':select.attr('selectedTmpText'),'selectedValue':select.attr('selectedTmpValue')});
						//select.attr({'selectedTmpText':'','selectedTmpValue':''});
					//	selectedvalue.attr({'value':select.attr('selectedTmpValue')});
			...
			$(document).click(function(){select.attr({'selectedTmpText':'','selectedTmpValue':''});objid.hide();});
			$(window).resize(function(){select.attr({'selectedTmpText':'','selectedTmpValue':''});objid.hide();});
		});
	};
